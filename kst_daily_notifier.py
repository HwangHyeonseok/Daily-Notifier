...
"""
KST Daily Notifier
- 매일 지정한 시간에 팝업 알림을 띄우는 간단한 윈도우 프로그램
- 일정은 로컬 JSON 파일에 저장되어 실행 재시에도 유지됩니다.
- 확인(OK) 버튼을 누르면 팝업이 닫히고, 일정을 삭제하기 전까지 매일 같은 시간에 재알림합니다.
- 확인 주기(폴링 간격)는 사용자가 지정 가능하며, 기본 30초입니다.
- 폴링 특성상 최대 지연을 보정하기 위해, 실제 알림 발생 시각은
  [설정한 알림시간 - (5분 + 확인 주기)] 로 앞당겨 팝업을 띄웁니다.
  (예: 09:00, 주기 30초 → 08:54:30에 팝업 시작)
"""
import json
import os
import threading
...
def get_data_dir() -> Path:
    # Windows: %APPDATA%\KSTDailyNotifier
    if platform.system() == "Windows":
        base = os.getenv("APPDATA") or str(Path.home() / "AppData" / "Roaming")
        p = Path(base) / APP_NAME
    # macOS: ~/Library/Application Support/KSTDailyNotifier
    elif platform.system() == "Darwin":
        p = Path.home() / "Library" / "Application Support" / APP_NAME
    # Linux: ~/.config/KSTDailyNotifier
    else:
        p = Path.home() / ".config" / APP_NAME
    p.mkdir(parents=True, exist_ok=True)
    return p

DATA_FILE = str(get_data_dir() / "schedules.json")
...
class NotifierApp:
    def __init__(self, root):
        self.root = root
        self.root.title("KST Daily Notifier (일정 알림)")
    ...
    def build_ui(self):
        ...
        columns = ("title", "time", "active", "last")
        ...
        self.tree.column("last", width=150, anchor="center")
        ...
        hint = (
            ...
            "알림 시간을 현재 시간보다 이후 시간으로 입력하면 다음 날 알림이 뜹니다."
        )
    ...
    def refresh_tree(self):
        ...
        for idx, s in enumerate(self.schedules):
            self.tree.insert("", "end", iid=str(idx),
                             values=(s.title, s.time_str, "예" if s.active else "아니오", s.last_fired_date or "-"))
    ...
    def add_schedule(self):
        ...
        if not self._validate_time(tstr):
            ...
            return
        self.schedules.append(Schedule(title=title, time_str=tstr))
        self.save_schedules()
        self.refresh_tree()
        self.title_var.set("")
        self.time_var.set("")
    ...
    def _check_and_alert(self):
        ...
        for s in self.schedules:
            ...
            # 지금 팝업을 띄워보자. (확인 주기 지연/프로그램 시작 시각 등을 고려)
            ...
            # 만약 target_dt도 이미 지난 경우라면, 오늘은 건너뛰고 내일 다시
            # (내일 루프에서 today_str이 바뀌면 다시 후보가 됨)
            # 별도 처리 불필요
